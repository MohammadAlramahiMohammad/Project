        ...
        static void Login()
        {
            ...
            else
            {
                ...
                if (user.UserType == "Admin")
                {
                    ...
                    BookIssueViewModel bookIssueViewModel = new BookIssueViewModel();
                    int userIndex = 3;
                    int bookIndex = 4;
                    bool LibIsOn = true;
                    while (LibIsOn)
                        ...
                        if (option == 1)
                        {
                            ...
                            while (userLoop)
                            {
                                ...
                                if (userOp == 1)
                                {
                                    ...
                                    while (addLoop)
                                    {
                                        string uname = otherMethods.StringInput("enter the user name : ");
                                        string pass = otherMethods.StringInput("enter the password of the user : ");
                                        String type = otherMethods.StringInput("enter the type of the user (Student OR Admin) : ");
                                        User users = new User(uname.ToLower(), pass.ToLower(), type);
                                ...
                                else if (userOp == 2)
                                {
                                    ...
                                    while (userLoopTwo)
                                    {
                                        ...
                                        if (userViewModel.NotNullList())
                                        {
                                            ...
                                            if (isInrange)
                                            {
                                                string uname = otherMethods.StringInput("edit the user name : ");
                                                string pass = otherMethods.StringInput("edit the password of the user : ");
                                                String type = otherMethods.StringInput("edit the type of the user (Student OR Admin) : ");
                                                User users = new User(uname.ToLower(), pass.ToLower(), type);
                ...
                try
                {
                    ...
                    foreach (string line in lines)
                    {
                        ...
                        foreach (User users in users)
                        {
                            if (users == null) ;
                            continue;
                             user = new User(users.Username, users.Password, users.UserType);
                        }
                        Console.Write("Done");
...
public class BookIssueViewModel
{
    ...
    public string IssuedOnHandBooks(int res)
    {
        ...
        for (int i = 0; i < bookIssues.Count(); i++)
        {
            ...
            {
                str = str + $"User : {bookIssues[i].UserId.Username} has book called: {bookIssues[i].BookId.Title} by {bookIssues[i].BookId.Author}  \n";
            }
        }
        return str;
    }
    public string IssuedBook(int index)
    {
        ...
        foreach (BookIssue bookIssue in bookIssues)
        {
            ...
            {
                str = str + $"Issued id :{bookIssue.IssueId} User : {bookIssue.UserId.Username} Book Name : {bookIssue.BookId.Title} by {bookIssue.BookId.Author} IsReturn:{bookIssue.IsReturned} \n";
            }
    ...
    public void View()
    {
        foreach (BookIssue bookIssue in bookIssues)
        {
            Console.Write($"user name : {bookIssue.UserId.Username} book name : {bookIssue.BookId.Title} ");
        }
    }
    public void BookIssueView()
    {
        ...
        foreach (BookIssue bookIssue in bookIssues)
        {
            str = str + $"issued id: {bookIssue.IssueId} user name : {bookIssue.UserId.Username} book title : {bookIssue.BookId.Title} by {bookIssue.BookId.Author} \n";
        }
    ...
    public string CurrentIssuedBook(int res)
    {
        ...
        for (int i = 0; i < bookIssues.Count(); i++)
        {
            ...
            {
                str = str + $"Book Name : {bookIssues[i].BookId.Title} Author:{bookIssues[i].BookId.Author} is Issued to: {bookIssues[i].UserId.Username}   \n";
            }
        }
        return str;
    }
    public string ReturnedBookHistory(int index)
    {
        ...
        foreach (BookIssue bookIssue in bookIssues)
        {
            ...
            {
                str = str + $"Book Name :{bookIssue.BookId.Title} Author:{bookIssue.BookId.Author} returned by {bookIssue.UserId.Username}   \n";
            }
        }
        return str;
    }
    public void IssuedList()
    {
        ...
        foreach (BookIssue bookIssue in bookIssues)
        {
            ...
            str = str + $"issued book id: {bookIssue.IssueId} name of user : {bookIssue.UserId.Username} book name : {bookIssue.BookId.Title} written by {bookIssue.BookId.Author} IsReturned:{bookIssue.IsReturned}\n";
    ...
    {
        Console.WriteLine($"Issued book id: {bookIssues[bookIssues.Count() - 1].IssueId} User Name : {bookIssues[bookIssues.Count() - 1].UserId.Username} Book Name : {bookIssues[bookIssues.Count() - 1].BookId.Title} Written by {bookIssues[bookIssues.Count() - 1].BookId.Author} is issued");
    }
...
internal class UserViewModel : AbstractMethod, IUser
{
    public static List<User> users = new List<User>
        {
            new User("ibrahim","ibrahim", "Admin"),
            new User("geek", "nerd", "Admin")
    ...
    {
        User user = (User)obj;
        new User(user.Username, user.Password,user.UserType);
    ...
    public override void Edit(int index, object obj)
    {
        ...
        for (int i = 0; i < users.Count(); i++)
        {
            if (users[i].UserId == index)
            {
                ...
                users[i].UserType = user.UserType;
            }
        }
    ...
    public override void Search(string search)
    {
        ...
        for (int i = 0; i < users.Count(); i++)
        {
            ...
            {
                ans = ans + "Username: " + users[i].Username + "Usertype: " + users[i].UserType + "\n";
            }
            else ans = "user not found";
    ...
    public void view()
    {
        Console.WriteLine($"User Id :{users[users.Count() - 1].UserId} User name : {users[users.Count() - 1].Username} is added");
    }
